'use strict'

// Write your answers inside this file, at the places where it's indicated by
// the comments.

// 1. Suppose that we wanted to create a browser-based calendar program for
// keeping track of things. Identify at least two entites such a program might
// need and describe attributes and methods of each.
//
// EXAMPLE
// Description: Our app manages car sales.
// Entity: Car
// Attributes: make, model, year
// Methods: drive, brake, park
//
// Entity: SalesPerson
// Attributes: name, commissionRate
// Methods: sell, call

// Answer Starts Here
Description: Our app is a calendar program

Entity: Year
Attributes: contains all the months in one Year
Methods: change to new year,

Entity: months
Attributes: contains all the days in one months
Methods: Change to a 30 or 31 day month, or 28 day month,

Entity: day
Attributes: contains all events for that day
Methods: add new event,
// Answer Ends Here

// 2. Create an object literal that lines up with the following `owner` entity
// description. Store it in the variable below. Be sure to give it reasonable
// values for each of its properties.
//
// Entity: Owner
// Attributes: name, address, numberOfPets

// Replace null with your answer
const owner = [

{
  name: 'Rick',
  address:'Earth C-137',
  numberOfPets: [1, 'dog', 'snuffles']
}
]
// 3. Create an object literal that lines up with the following `pet` entity
// description. Store it in the variable `pet`, below.

// Entity: Pet
// Attributes: name, species, breed, vocalization
// Methods: vocalize

// Replace null with your answer
const pet = [
{
  name: 'Snuffles',
  species: 'dog'
  breed: 'Terrier'
  vocalization: 'bark', 'bork'
}
]

// 4. Create a constructor function for pet objects, as described above. The
// constructor function should be responsible for setting the values of `name`,
// `species`, `breed`, and `vocalization`.

const Pet = {
  /* Your answer here */
function Pet (name, species, breed, vocalization) {
  this.name = name;
  this.species = species;
  this.breed = breed;
  this.vocalization = vocalization
}
const thePet = new Pet('Snuffles', 'Dog', 'Terrier', 'Bark')
}

// 5. Attach a `vocalize` method to the Pet's prototype.  The `vocalize` method
// should return the pet's vocalization attribute.

/* your answer here */
vocalize: function () {

}

// Don't change this
module.exports = {
  answerTwo: owner,
  answerThree: pet,
  answerFour: Pet
}
