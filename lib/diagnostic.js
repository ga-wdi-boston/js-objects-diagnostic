'use strict';

// Write your answers inside this file, at the places where it's indicated by
// the comments.

// 1. Suppose that we wanted to create a browser-based calendar program for
// keeping track of things. Identify at least two entites such a program might
// need and describe attributes and methods of each.
//
// EXAMPLE
// Description: Our app manages car sales.
// Entity: Car
// Attributes: make, model, year
// Methods: drive, brake, park
//
// Entity: SalesPerson
// Attributes: name, commissionRate
// Methods: sell, call

// Answer Starts Here
//
//Description: Our app keeps track of dates and schedules.
//Entity: calendar
//Attributes: day, month, year
//Methods: return today's date, return appointments for today
//
// Entity: schedule
// Attibutes: time, place, reason
// Methods: create appointments, cancel appointments
// Answer Ends Here

// 2. Create an object literal that lines up with the following `owner` entity
// description. Store it in the variable below. Be sure to give it reasonable
// values for each of its properties.
//
// Entity: Owner
// Attributes: name, address, numberOfPets

// Replace null with your answer
let owner = {
  name: 'Lauren',
  address: '125 Summer Street, Boston, MA',
  numberOfPets: 1,
};

// 3. Create an object literal that lines up with the following `pet` entity
// description. Store it in the variable `pet`, below.

// Entity: Pet
// Attributes: name, species, breed, vocalization
// Methods: vocalize

// Replace null with your answer
let pet = {
  name: 'Briscoe',
  species: 'cat',
  breed: 'ragdoll',
  vocalization: 'meow',
  vocalize: function () {
    return this.vocalization;
  },
};

// 4. Create a constructor function for pet objects, as described above. The
// constructor function should be responsible for setting the values of `name`,
// `species`, `breed`, and `vocalization`.

const Pet = function Pet(name, species, breed) {
  this.name = name;
  this.species = species;
  this.breed = breed;
  this.vocalization = [];
};

const Vocalization = function Vocalization(date, soundsLike, mood) {
  this.date = date;
  this.soundsLike = soundsLike;
  this.mood = mood;
};

// 5. Attach a `vocalize` method to the Pet's prototype.  The `vocalize` method
// should return the pet's vocalization attribute.

Pet.prototype.vocalize = function () {
  let vocalization = this.vocalization;
  let result = 0;
  for (let i = 0; i < vocalization.length; i++) {
    result =  vocalization[i].soundsLike;
  }

  return result;
};

Pet.vocalization.push(new Vocalization('2017-01-17', 'meow', 'happy'));
Pet.vocalization.push(new Vocalization('2017-01-17', 'hiss', 'mad'));

// Don't change this
module.exports = {
  answerTwo: owner,
  answerThree: pet,
  answerFour: Pet,
};
