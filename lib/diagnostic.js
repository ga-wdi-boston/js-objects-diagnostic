...
//  Description: this app is a browser-based calendar for keeping track of things
    Entity: calendar
    Attributes: year, month, day, events
    Methods: remind, changeYear, changeMonth, changeDay

    Entity: events
    Attributes: title, description, year, month, day, reminderSchedule,
      recurrence
    Methods: none
...
const Cujo = new Pet(pet.name, pet.species, pet.breed, pet.vocalization)
console.log(Cujo)

// 5. Attach a `vocalize` method to the Pet's prototype.  The `vocalize` method
// should return the pet's vocalization attribute.

Pet.prototype.vocalize = function (petDesired) {
  return petDesired.vocalization
};
Pet.prototype.vocalize(pet)
